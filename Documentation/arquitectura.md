...
```
┌─────────────────┐    HTTP        ┌─────────────────┐    HTTP/JSON    ┌─────────────────┐    ORM/SQL    ┌─────────────────┐
│                 │   Port 80/443  │                 │    API REST     │                 │   Alembic     │                 │
│    Browser      │ ◄─────────────►│ Frontend Server │ ◄─────────────► │    Backend      │ ◄───────────► │   PostgreSQL    │
│   (Cliente)     │                │  (Python HTTP)  │   Port 8000     │   (FastAPI)     │               │   Database      │
│                 │                │   Port 9000     │                 │                 │               │                 │
└─────────────────┘                └─────────────────┘                 └─────────────────┘               └─────────────────┘
        │                                   │                                   │                                   │
        │                                   │                                   │                                   │
        ▼                                   ▼                                   ▼                                   ▼
┌─────────────────┐                 ┌─────────────────┐                 ┌─────────────────┐               ┌─────────────────┐
│  Browser APIs   │                 │  Python Server  │                 │  Python Libs    │               │  Data Storage   │
│  - localStorage │                 │  - http.server  │                 │  - SQLAlchemy   │               │  - Tables       │
│  - sessionStorage│                 │  - CORS headers │                 │  - Pydantic     │               │  - Indexes      │
│  - Fetch API    │                 │  - Static files │                 │  - Passlib      │               │  - Constraints  │
└─────────────────┘                 └─────────────────┘                 └─────────────────┘               └─────────────────┘
```
...
```
accesibility-things/
├── start-frontend.py        # 🟢 Servidor HTTP Python para frontend (puerto 9000)
├── start-frontend.ps1       # ⚪ Script PowerShell (vacío)
├── server.js               # ⚪ Servidor Node.js (vacío)
├── docker-compose.yml      # 🐳 Orquestación Backend + PostgreSQL
├── package.json           # 📦 Configuración Node.js del proyecto
├── nginx.conf             # 🌐 Configuración Nginx para producción
├── LICENSE                # 📄 Licencia del proyecto
├── README.md              # 📖 Documentación principal
├── arquitectura.md        # ⚪ Arquitectura (vacío - usar Documentation/)
...
└── Documentation/         # 📚 Documentación del proyecto
    ...
    ├── *.pdf            # Documentos de entrega
    └── presentaciones/   # Materiales de presentación
```

**Leyenda**:
- 🟢 **Activo/Completo** - Archivo funcional y documentado
- 🐳 **Docker** - Containerización
- ⚪ **Vacío/Placeholder** - Archivo presente pero sin contenido
...
El sistema utiliza una arquitectura de microservicios con separación clara entre frontend y backend:

```
┌─────────────────────────────────────────────────────────────────────┐
│                        DESARROLLO LOCAL                              │
├─────────────────────────────────────────────────────────────────────┤
│  Frontend Server        Backend API           Database              │
│  ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐    │
│  │ Python HTTP     │   │ FastAPI         │   │ PostgreSQL      │    │
│  │ Port: 9000      │   │ Port: 8000      │   │ Port: 5432      │    │
│  │ Static Files    │   │ REST API        │   │ Data Storage    │    │
│  │ CORS Enabled    │   │ JWT Auth        │   │ Migrations      │    │
│  └─────────────────┘   └─────────────────┘   └─────────────────┘    │
└─────────────────────────────────────────────────────────────────────┘
```

### Servidor Frontend de Desarrollo

#### Características del Servidor Python
```python
# start-frontend.py - Servidor HTTP personalizado para desarrollo
...
**Funcionalidades**:
- ✅ Servidor HTTP en puerto 9000
- ✅ Servir archivos estáticos desde `/frontend`
- ✅ Headers CORS habilitados para desarrollo
- ✅ Headers de seguridad básicos
- ✅ Manejo de requests OPTIONS
- ✅ Logging personalizado con emojis
...
**URLs disponibles**:
- `http://localhost:9000/` - Página principal
- `http://localhost:9000/catalogo.html` - Catálogo de productos
- `http://localhost:9000/carrito.html` - Carrito de compras
- `http://localhost:9000/profile.html` - Perfil de usuario
- `http://localhost:9000/login.html` - Autenticación
...
```yaml
# docker-compose.yml - Solo Backend y Base de Datos
services:
  backend:
    build:
      context: ./backend
...
**Nota**: El frontend se ejecuta independientemente usando `start-frontend.py` para facilitar el desarrollo y permitir modificaciones en tiempo real sin reconstruir containers.
...
**2. Iniciar Frontend (en terminal separado):**
```bash
python start-frontend.py
```
